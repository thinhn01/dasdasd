...
module.exports.run = async function({ api, args, event }) {
  ...
  if(args[0] == "list") {
    ...
    const res = await axios.get('https://api.nguyenmanh.name.vn/taoanhdep/list');
     var list = res.data.listAnime.length;
     var msg = [];
    var char = args[1]
    for(var i = 0; i < res.data.listAnime.length; i++) {
       if(char == res.data.listAnime[i].name) {
         msg += `${i + 1 -1} `;
       } 
     }
     
     return api.sendMessage(`» ID của ${char} là: ` + msg, threadID, messageID);
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    case "characters": {
      ...
      var name = res.data.listAnime[event.body].name
    ...
    case "color": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`» Reply tin nhắn này để nhập màu nền (Màu tiếng Anh)`, threadID, (error, info) => {
        return global.client.handleReply.push({
          type: 'create',
          name: this.config.name,
          author: senderID,
