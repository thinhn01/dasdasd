...
var full = [];
        ...
        const slotItems = ["ga", "tom", "bau", "cua", "ca", "nai"];
            const moneyUser = (await Currencies.getData(event.senderID)).money;
                var moneyBet = parseInt(args[1]);
                    ...
                    if (isNaN(moneyBet) || moneyBet <= 0) return api.sendMessage("[Tobi] => Số tiền đặt cược không được để trống hoặc là số tiền âm", event.threadID, event.messageID);
                if (moneyBet > moneyUser) return api.sendMessage("[Tobi] => Số tiền bạn đặt lớn hơn số dư của bạn!", event.threadID, event.messageID);
            if (moneyBet < 1000) return api.sendMessage("[Tobi] => Số tiền đặt không được dưới 1000 đô!", event.threadID, event.messageID);
                ...
                switch (args[0]) {
                    ...
                    case "tôm":
                        case "Tôm": itemm = "tom";
                                icon = '🦞';
                            break;
                                default: return api.sendMessage("[Tobi] => Hãy Bấm : /baucua [bầu/cua/cá/nai/gà/tôm] [số tiền]",event.threadID, event.messageID);
                }      
                await get(number[0],number[1],number[2]);
            api.sendMessage({body:"[Tobi] => Đang Đập, À Không Đang Lắc!",attachment: createReadStream(__dirname + "/cache/baucua.gif")},event.threadID,async (error,info) => {
    ...
    var array = [number[0],number[1],number[2]];
        ...
        if (array.includes(itemm)) {
            ...
            else return api.sendMessage("[Tobi] => Lỗi ! Code : XX1N",event.threadID,event.messageID);
        } else  {
